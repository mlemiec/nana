"""Unit tests for src/actions/housekeeping.py"""
import pytest
...
def tmp_vault(tmp_path):
    """Create a temporary vault with messy and clean files."""
    raw = tmp_path / "raw"
    raw.mkdir()
...
def test_only_renames_messy_files(tmp_vault):
    """clean-note.md should not be renamed."""
    with patch("actions.housekeeping.send_request_to_model", return_value="machine_learning_basics"):
        tidy_vault(tmp_vault, {}, LOCALE)

    # Clean note should still exist
    assert (tmp_vault / "raw" / "clean-note.md").exists()


def test_renames_untitled_files(tmp_vault):
    """Files starting with 'Untitled' should be renamed."""
    with patch("actions.housekeeping.send_request_to_model", return_value="machine_learning_basics"):
        tidy_vault(tmp_vault, {}, LOCALE)
...
def test_renames_web_clip_files(tmp_vault):
    """Files starting with 'web-clip' should be renamed."""
    with patch("actions.housekeeping.send_request_to_model", return_value="python_intro"):
        tidy_vault(tmp_vault, {}, LOCALE)
...
def test_no_mess_returns_early(tmp_vault):
    """If no messy files found, function returns without calling the model."""
    raw = tmp_vault / "raw"
    # Remove messy files
    (raw / "Untitled_1.md").unlink()
    (raw / "web-clip-123.md").unlink()

    with patch("actions.housekeeping.send_request_to_model") as mock_model:
        tidy_vault(tmp_vault, {}, LOCALE)
        mock_model.assert_not_called()


def test_uses_custom_target_dir(tmp_path):
    """Should use target_dir instead of vault/raw when provided."""
    custom = tmp_path / "custom"
    custom.mkdir()
    (custom / "Untitled.md").write_text("Content")

    with patch("actions.housekeeping.send_request_to_model", return_value="new_name"):
