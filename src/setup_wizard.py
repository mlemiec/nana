...
def setup_settings(console: Console, lang: LocaleManager):
    ...
    model_name = select(
        ...
        choices=free_models,
        style=None
    ).ask() or FREE_MODELS_FALLBACK[0]

    env_path = Path(".env")
    set_key(str(env_path), "NANA_VAULT_PATH", str(vault_path))
    set_key(str(env_path), "NANA_API_KEY", api_key)
    set_key(str(env_path), "NANA_MODEL_NAME", model_name)
